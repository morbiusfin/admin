...
(function () {
  ...
  function renderTable(filter) {
    ...
    rows.forEach(function (l) {
      ...
      html += '<div class="ad-row row-' + esc(tier) + '" data-email="' + esc((l.email || "").toLowerCase().trim()) + '">'
    ...
    document.querySelectorAll(".ad-row").forEach(function (row) {
      ...
      row.querySelector('[data-act="toggle"]').onclick = function () { toggleBlock(email); };
      row.querySelector('[data-act="save"]').onclick = function () { saveRow(email, row); };
      row.querySelector('[data-act="plus30"]').onclick = function () { shiftValidade(email, row, 30); };
      row.querySelector('[data-act="plus1y"]').onclick = function () { shiftValidade(email, row, 365); };
      row.querySelector('[data-act="vitalicio"]').onclick = function () { setVitalicio(email, row); };
    });
  }
  var _searchT = null;
  ...
  function byEmail(email) { return _all.find(function (x) { return (x.email || "").toLowerCase().trim() === email; }); }
  // TODAS as escritas por EMAIL (mesma chave que o APP lê) + .select() pra confirmar que mudou de fato.
  async function toggleBlock(email) {
    var l = byEmail(email); if (!l) return;
    var novo = l.status === "bloqueado" ? "ativo" : "bloqueado";
    var r = await client().from("licencas").update({ status: novo }).eq("email", email).select();
    if (r.error) { alert("Falha: " + r.error.message); return; }
    if (!r.data || !r.data.length) { alert("Nada gravado — email não encontrado na tabela: " + email); return; }
    l.status = novo; renderTable($("#adSearch") ? $("#adSearch").value : "");
  }
  async function saveRow(email, row) {
    ...
    var r = await client().from("licencas").update({ plano: plano, validade: validade }).eq("email", email).select();
    if (r.error) { btn.textContent = "Salvar"; alert("Falha: " + r.error.message); return; }
    if (!r.data || !r.data.length) { btn.textContent = "Salvar"; alert("Nada gravado — email não encontrado na tabela: " + email); return; }
    var l = byEmail(email); if (l) { l.plano = plano; l.validade = validade; }
  ...
  async function shiftValidade(email, row, dias) {
    var l = byEmail(email); if (!l) return;
    var base = new Date();
    ...
    var r = await client().from("licencas").update({ validade: novaVal }).eq("email", email).select();
    if (r.error) { alert("Falha: " + r.error.message); return; }
    if (!r.data || !r.data.length) { alert("Nada gravado — email não encontrado: " + email); return; }
    l.validade = novaVal;
  ...
  async function setVitalicio(email, row) {
    var l = byEmail(email);
    var r = await client().from("licencas").update({ validade: null }).eq("email", email).select();
    if (r.error) { alert("Falha: " + r.error.message); return; }
    if (!r.data || !r.data.length) { alert("Nada gravado — email não encontrado: " + email); return; }
